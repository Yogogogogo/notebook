...
之后保存一下，后续做PCB的调用需要的是一个后缀psm的文件，基本保存一下就有了，没有的话File—>Create Symbol 

### PCB Editor软件创建插件封装(插针为例)

插件封装的创建会比贴片焊盘麻烦一点，需要添加热风焊盘和反焊盘

示例中的插针是2.54mm的，通孔直径1mm，焊盘直径1.3mm

弹幕有位兄弟建议打开pad editor先改单位 ，padstack usage选择Thru Pin,pad geometry选择Circle,一般情况下这个通孔都是圆形的

![image-20251120235129314](./assets/image-20251120235129314.png)

钻孔的属性只需要关注上图三个参数即可，其他的不需要关注

secondary Drill是盲埋孔等的设计，一般来说用不上，可以不用管

Drill symbol是表示钻孔符号的，视频中是按照一个标准孔径表来填写的

![image-20251120235549904](./assets/image-20251120235549904.png)
