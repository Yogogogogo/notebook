...
1、选择tools–Footprint Manager(快捷键：T+G）
...
## AD中关于NET Only one pin的解决方法

![image-20260303133619709](assets/image-20260303133619709.png)

如上图所示

FG和P这两个会报错，但是只要加了测试点或者其他的器件如OUT1和OUT2就不会报错了

## 尽量使用机械一层做板框层

如果使用keepout做板框层，在使用拼版阵列的时候不会显示keepout层的线条，导致没有外形，但是疑惑的是迷吉哥的拼版教程里使用的是keepout层做板框，使用拼版阵列可以显示板框，版本是24.1，经测试，是版本问题，AD22拼版阵列不显示keepout的线条，AD24可以

但是使用机械一层做板框层的话，在铺铜的时候需要注意要设计Board outline Clearance

keepout层的作用是限制铺铜及走线

## 工程路径太深或者路径有中文点击DRC报错界面的报错信息不会跳转

在画10025A风扇的板子时发现不知道是因为路径有中文还是因为工程路径太深导致的点击DRC报错界面的报错信息不会跳转到PCB的相应位置，我把这个工程单独放在桌面后这个问题就解决了
...
![image-20260401093922746](assets/image-20260401093922746.png)

快捷键F5可以快捷切换是否使用网络颜色覆盖，使铺铜在棋盘状和一种颜色中转换

## 导入dxf文件问题

注意在CAD中不可以自己改坐标原点，只能把图形使用命令挪动到原点

还有就是一定单位选择对，在AD的界面一定要选择CAD中的单位，一般是mm，要不图形会缩在一起

在AD的界面中需要点一下那个选择坐标的，这个坐标貌似是绝对坐标，不是你设置的原点为标准的

关闭图层这个选项可能会有用，目前没试出效果，但是我建议以后都把图形选中重新导出为新文件，首选直接复制，这个最省心

以下是AI回答：

> ### 1. 最直接的原因：CAD 图形本身离绝对原点太远
>
> 很多工程师在 CAD 里随手画板框，没有关注图形的绝对坐标。如果板框在 `(50000, 60000)` 毫米这种位置，导入 AD 后它会老老实实呆在那个遥远的地方，AD 的默认视图里根本看不到。
>
> **在 AutoCAD 中检查并修正：**
>
> - 打开 DXF，命令行输入 `UCS` → `W`，确保处于**世界坐标系**。
> - 输入 `ID`，点一下板框的某个角点，查看左下角坐标值。如果数值非常大，就是问题所在。
> - 选中整个板框图形，输入 `MOVE`：
>   - 基点：点选板框上你想放在 AD 原点的角（比如左下角）。
>   - 目标点输入：`0,0`（回车）。
> - 再运行 `PURGE`（清理）并保存为 DXF（建议存为 **AutoCAD 2000/LT2000 DXF**，兼容性更好）。
>
> 这样处理完，导入时选择“文件原点”就能让板框精确落在 AD 原点了。
>
> ### 2. 导入 AD 时“原点”选项设置不当
>
> AD 导入 DXF 向导中有一个 **“原点 (Origin)”** 设置页面，这里常被忽视：
>
> - **文件指定 (File specified)**：严格按 DXF 内部坐标放置。如果你没在 CAD 里移动图形到原点，它就会“消失”在远处。
> - **自动 (居中) (Auto (Centered))**：AD 自动计算所有导入图形的几何中心，把该中心对齐到 PCB 的绝对原点 `(0,0)`。**这是最稳妥、最不容易“跑飞”的选项**——你总能导入后在可见区域内看到它，然后再整体移动。
>
> **建议做法**：
> 导入时单位选对（毫米/密尔），在原点页面先选择 **自动(居中)**，确保图形出现在视野内。然后回到 PCB 编辑器，选中导入后的板框线条，使用 **编辑 → 移动 → 移动选中对象**，再用跳转命令 `J` → `O`（跳转到原点）或直接输入 `0,0` 作为偏移目标，把板框精确放到你需要的位置。
>
> ### 3. DXF 内存在“幽灵图元”导致边框异常
>
> 有时文件里含有一个极远处的点、隐藏块或空文字，导致图形范围被撑得巨大。如果用“自动(居中)”计算出的中心点严重偏离，板框仍可能偏在一边。
>
> - **解决方法**：在 CAD 中只保留板框所在的图层，其余图层全部关闭或删除，再保存。或者用 `Ctrl+A` 全选图形后，按住 Shift 手动剔除板框，把多余部分删掉，再执行移动和清理。
>
>   ## 4. 块与外部参照未炸开
>
>   如果板框在 CAD 里是“块”，且该块的插入点离几何图形很远，导入 AD 后也会整体位移。
>
>   - 解决方法：在 CAD 中用 `EXPLODE` 把板框图元全部炸开，直到全是直线/多段线，再执行第 1 步的移动和清理。
>
>     ### 5. 单位不匹配引发的间接问题（不太影响原“点”，但会改变相对感）
>
>     CAD 用英寸绘制，AD 导入时选了毫米，图形会被缩放 25.4 倍。虽然坐标数值不变，但如果数值本身偏大，缩放后视觉上离原点更远。**所以请统一单位**：CAD 里用 `UNITS` 设好单位，画板框按实际尺寸 1:1 绘制，AD 导入时选择对应的单位（毫米）

> ### CAD 有两套坐标系，AD 只认其中一套
>
> AutoCAD 里存在两套坐标系：
>
> - **世界坐标系 (WCS)**：这是图形在宇宙中“真实的”绝对位置，恒定不变。
> - **用户坐标系 (UCS)**：这是你为了方便画图，临时移动、旋转的“工作台面”和“坐标系图标”。
>
> **关键是：DXF 文件里，图形的位置永远是按世界坐标 (WCS) 保存的。UCS 只是一个视图状态，其他软件通常不认。**
>
> ------
>
> ### 1. 为什么“将图形挪动到原点”就没问题？
>
> 你说的是用 **MOVE** 命令，把图形实体从远处真正搬到了 `0,0` 点。
>
> - 图形的世界坐标被真实修改了，变成了 `0,0` 附近的数值。
> - 保存成 DXF 后，这些新坐标就写入了文件。
> - Altium Designer 读取这些坐标，图形自然就乖乖出现在原点。
>
> ### 2. 为什么“将原点设置在图形中心”会出问题？
>
> 你很可能用的是 **UCS** 命令，把用户坐标系的原点“放”在了图形中心。这时你会看到坐标显示变为 `0,0`，误以为图形已经归位，但实际上：
>
> - **图形的世界坐标纹丝未动**，可能还在 `(50000, 60000)` 这种遥远的地方。
> - 你看到的 `0,0`，只是 UCS 的当前原点，是世界坐标的一个偏移。
> - DXF 文件保存的是世界坐标 `(50000, 60000)`，UCS 的那点偏移信息 AD 完全忽略。
> - 结果就是，导入 AD 后，图形会飞回它原本的世界坐标位置，当然就不在可视区域里了。
>
> ------
>
> ### 正确归位的唯一方法
>
> 要准备给 AD 用的 DXF，永远只能做一件事：**在世界坐标系下，用 MOVE 命令把图形本身移动到绝对原点 `0,0`。**
>
> 操作步骤如下，这样最可靠：
>
> 1. 命令行输入 **UCS** → 回车 → 输入 **W** → 回车，确保回到世界坐标系。
> 2. 输入 **ID** 命令，点一下你想作为参考的板框角点，看看左下角坐标是不是一个巨大的数。
> 3. 选中整个板框图形，输入 **MOVE** 回车。
> 4. 基点：精确点选那个参考角点。
> 5. 目标点：直接输入 **`0,0`** 回车。
> 6. 运行 **PURGE** 清理一下，另存为 **2000 版 DXF**。
>
> 这样处理后，AD 导入时即使选“文件指定”原点，也会百分百落在绝对原点上。

有时候CAD移动图形发现没法移动，那可能就是动态输入打开了，输入的坐标是偏移量，可以在右下角三个横杠那里先把动态输入勾选让他显示出来再关闭

![image-20260617103311724](assets/image-20260617103311724.png)
